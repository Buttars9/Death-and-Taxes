export function getEligibleCredits({
  ...
  // 🧓 Senior Bonus Credit (OBBBA-specific)
  if (age >= 65) {
    const isMarried = filingStatus === 'marriedJointly' || filingStatus === 'qualifyingWidow';
    const baseAmount = isMarried ? 12000 : 6000;
    const phaseOutStart = isMarried ? 300000 : 150000;
    const excess = Math.max(income - phaseOutStart, 0);
    const reduction = (excess / 50000) * 0.1; // 10% per $50k over
    const amount = Math.max(baseAmount * (1 - reduction), 0);
    credits.push({ type: 'senior_bonus', amount, note: amount > 0 ? 'OBBBA senior bonus applied' : 'Phased out due to income' });
  }
